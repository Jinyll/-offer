...
//遍历每一个元素，（1）如果已经到达目标路径结束处，则返回正确
//（2）否则如果当前元素不匹配或出界返回错误；
//（3）当前元素的坐标未出界且和当前str相同且没访问过，则访问当前元素，通过pathlength后移目标路径下标，并递归尝试四个方向
//（3.1）如果都不对，则通过当前元素不会匹配路径，设置未访问，目标路径下标前移，返回错误；（3.2）否则返回正确
class Solution {
public:
